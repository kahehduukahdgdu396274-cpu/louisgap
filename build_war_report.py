...
_re_war = re.compile(r"str(003|006|009|010|011|012|013|014)(open|close)")
VALID_TRADE_STRATEGIES = frozenset({"003", "006", "009", "010", "011", "012", "013", "014"})
POOL_MEMBERS = frozenset({"009", "010", "011", "012"})
INDEPENDENT_STRATEGIES = VALID_TRADE_STRATEGIES - POOL_MEMBERS
DEFAULT_EQ = {"003": 200, "006": 200, "009": 250, "010": 250, "011": 250, "012": 250, "013": 200, "014": 200, "POOL_009_012": 250}

BASE_URL = "https://openapi.okx.com"
SYMBOL = "BTC-USDT-SWAP"
BASE_DIR = "/home/admin/okx_bot"
...
def equity_after_close_value(sid, eq):
    """平仓后权益快照：共享池记 POOL_009_012，其余记独立策略键。"""
    if sid in POOL_MEMBERS:
        return eq.get("POOL_009_012", eq.get(sid, DEFAULT_EQ.get("POOL_009_012", 250)))
...
def _rewind_equity_baseline(current_eq, closed_rows):
    ...
    for row in reversed(closed_rows):
        ...
        try:
            ...
            pnl = 0.0
        if sid in POOL_MEMBERS:
            eq["POOL_009_012"] = round(eq.get("POOL_009_012", DEFAULT_EQ["POOL_009_012"]) - pnl, 2)
        elif sid:
...
def _replay_equity_after_close(closed_rows, current_eq=None):
    ...
    for row in sorted(closed_rows, key=_row_time_ms):
        ...
        try:
            ...
            pnl = 0.0
        if sid in POOL_MEMBERS:
            running["POOL_009_012"] = max(
                1, round(running.get("POOL_009_012", DEFAULT_EQ["POOL_009_012"]) + pnl, 2)
            )
            replay_map[(row.get("ordId") or "").strip()] = running["POOL_009_012"]
        elif sid:
...
def prune_inflated_state_legs(base_dir=None):
    ...
    before = len(legs)
    filtered = [l for l in legs if (l.get("reason") or "") not in UNTRUSTED_LEG_REASONS]
    ...
    log(f"[govern] 剔除 OKX回补 膨胀腿 {before}→{len(filtered)}")
...
def pair_round_trips(fills):
    ...
    for _oid, fs in by_ord.items():
        leg = aggregate_ord_fills(fs)
        if leg and leg["strategy"] and leg["kind"] in ("open", "close"):
            legs.append(leg)
...
def reconcile_trades_from_state():
    ...
    for row in rows:
        ...
        oid = (row.get("ordId") or "").strip()
        if st == "持仓中":
            if oid in closed_open_ids:
                removed_open += 1
                log(f"[归因] 删除已平仓仍显示的持仓中 {row.get('策略')} open={oid}")
                # 同时清理 state.json 中残留的同 open_ord_id tranche（防死循环）
                try:
                    from position_state import load_state, save_state
                    st2 = load_state(BASE_DIR)
                    before = len(st2.get("tranches", []))
                    st2["tranches"] = [
                        t for t in st2.get("tranches", [])
                        if (t.get("open_ord_id") or "").strip() != oid
                    ]
                    after = len(st2.get("tranches", []))
                    if after < before:
                        save_state(st2, BASE_DIR)
                        log(f"[归因] 同步清理 state tranche open={oid} ({before}→{after})")
                except Exception as ex:
                    log(f"[归因] 清理 state tranche 失败: {ex}")
                continue
...
def finalize_close_leg_from_fills(leg):
    ...
    ok, reason = verify_realized_leg(leg, strict_open=True)
    if not ok:
    ...
    if not open_meta or not close_meta:
    ...
    et_ms = int(open_meta.get("fillTime") or 0)
...
def refresh_war_report_accurate(trigger_sid=None):
    """平仓后以 state.json + OKX 已校验 leg 为真源重建战报（优先准确，不拉全量 fills）。"""
    lock_fp = _war_lock()
    if lock_fp is None:
    ...
    try:
        ...
        reconcile_state_from_position_files(base_dir=BASE_DIR)
        clean_ghost_rows()
        dedup_trades_csv()
...
def _main_war_body():
    ...
    reconcile_state_from_position_files()
    clean_ghost_rows()
    dedup_trades_csv()
    reconcile_trades_from_state()
    rebuild_trades_from_state()
